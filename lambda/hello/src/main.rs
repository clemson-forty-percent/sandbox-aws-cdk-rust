...
async fn echo(event: ApiGatewayV2httpRequest, _: Context) -> Result<ApiGatewayV2httpResponse, Error> {
    ...
    let body = encodings::Body::from(format!("Hello from Rust! You've reached {}", event.raw_path.unwrap()));
...
mod tests {
    ...
    async fn echo_handles() {
        ...
        let response_body = encodings::Body::from("Hello from Rust! You've reached /");
